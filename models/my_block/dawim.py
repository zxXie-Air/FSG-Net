...
class DAWIM(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        # print("w_ll mean:", w_ll.mean().item(), "w_ll std:", w_ll.std().item())  # 打印权重的均值
        # 同样的方法计算LH, HL, HH的权重
        w_lh = self.se_lh(lh_interacted)
        # print("w_lh mean:", w_lh.mean().item(), "w_lh std:", w_lh.std().item())  # 打印权重的均值
        w_hl = self.se_hl(hl_interacted)
        # print("w_hl mean:", w_hl.mean().item(), "w_hl std:", w_hl.std().item())  # 打印权重的均值
        w_hh = self.se_hh(hh_interacted)
        # print("w_hh mean:", w_hh.mean().item(), "w_hh std:", w_hh.std().item())  # 打印权重的均值

        # 将计算出的权重应用到原始的、未交互的DWT分量上
        # 这里的操作是：加权(Hadamard积) + 残差连接，即 F' = F * w + F
        ...
        #     "F_t1_before": f1,
        #     "F_t2_before": f2,
        #     "LL_t1_before": f1_dwt_ll,
        #     "LL_t2_before": f2_dwt_ll,
        #     "LH_t1_before": f1_dwt_lh,
        #     "LH_t2_before": f2_dwt_lh,
        #     "HL_t1_before": f1_dwt_hl,
        #     "HL_t2_before": f2_dwt_hl,
        #     "HH_t1_before": f1_dwt_hh,
        #     "HH_t2_before": f2_dwt_hh,
        #     # --- 处理后 (After) ---
        #     "F_t1_after": f1_prime,  # 这是重构后的空间特征
        #     "F_t2_after": f2_prime,
        #     "LL_t1_after": f1_ll_p,  # 这是交互后的频域特征
        #     "LL_t2_after": f2_ll_p,
        #     "LH_t1_after": f1_lh_p,
        #     "LH_t2_after": f2_lh_p,
        #     "HL_t1_after": f1_hl_p,
        ...
        # }
        # #为了可视化，返回这个字典和原始输出
        # return (f1_prime, f2_prime), viz_outputs